...
/* const btnTestBox = document.querySelector('.btn__test-container');
const circle = document.querySelector('.x_circle');
const btnCircle = document.querySelector('.x_mask2');

btnTestBox.addEventListener('mousemove', (ev) => {
  circle.style.transform = `translate(${ev.clientX / 30 + `px`}, ${ev.clientY / 15 + `px`})`;
  btnCircle.style.clipPath = `circle(40px at ${Math.round(ev.clientX / 25) + `px`} calc(50% - ${Math.round(ev.clientY / 15) + `px`}))`;
})


btnTestBox.addEventListener('mouseout', (ev) => {
  circle.style.transform = `translate(0)`;
  btnCircle.style.clipPath = `circle(40px at 16px calc(50% + 0px))`;
}) */
...
/* $(document).ready(function () {
  $(document).on('mousemove', function (c) {
    $('.x_box').each(function () {
      var t = $(this).offset(), e = $(this).find('.x_circle'), r = $(this).find('.x_mask2'), i = c.pageX,
